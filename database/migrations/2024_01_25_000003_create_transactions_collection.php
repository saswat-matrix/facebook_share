...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::connection($this->connection)
                ->table('transactions', function (Blueprint $collection) 
                {
                    $collection->index('_id');
                    $collection->string('particular_id', 255);
                    $collection->string('charge_id', 255);
                    $collection->string('discount_id', 255);
                    ...
                    $collection->string('payment_id', 255)->nullable();
                    $collection->string('student_id', 255)->nullable();
                    $collection->enum('type', ['p', 'e', 't', 's', 'a'])
                                ->comment("p => promotion, e => enrolment, t => term payment, s => service payment, a => additional payment")
                    ...
                    $collection->enum('payment_status', ['c', 'i', 'd', 'p', 'pp', 'f', 'na'])
                                ->default('a')
        ...
        Schema::connection($this->connection)
                ->table('transaction_translations', function (Blueprint $collection) 
                {
                    $collection->index('_id');
                    $collection->longText('description')->nullable();
