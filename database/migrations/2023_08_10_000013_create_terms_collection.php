...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::connection($this->connection)
                ->table('terms', function (Blueprint $collection) 
                {
                    $collection->index('_id');
                    $collection->string('title', 255)->unique();
                    $collection->string('key', 255)->unique();
        ...
        Schema::connection($this->connection)
                ->table('term_translations', function (Blueprint $collection) 
                {
                    $collection->index('_id');
                    $collection->string('title', 255);
                    $collection->string('language', 10);
                    $collection->string('term_id');
                });        
    }
