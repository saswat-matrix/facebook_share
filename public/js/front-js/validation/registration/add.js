...
(function () {
  ...
  // check on load time, whether the content is scrolled to bottom of the terms and conditions content
  /*if($('#terms_content').scrollTop() + $('#terms_content').innerHeight() >= $('#terms_content')[0].scrollHeight) {
    //$('#terms_and_conditions').prop('disabled', false);
  } else {
    $('#terms_and_conditions').prop('checked', false);
    $('#terms_and_conditions').prop('disabled', true);
  } */

  // on scroll event, if scrolled down to the bottom of the terms and conditions content enable the terms and conditions checkbox,
  // else disable them
  ...
  // on document ready event if scrollbar is present in the terms and conditions content, disable the terms and conditions checkbox so that user has to scroll it
  // else scrollbar is absent, the checkbox is enabled
  // to enable or disable term condition checkbox based on the scrollbar of the term content
  $('#termModal').on('shown.bs.modal', function() {
    // check when term modal is shown
    var termContent = $('#termModal #terms_content');
    if (termContent[0].scrollHeight > termContent.innerHeight()) {
      // if term content has scrollbar, disable term condition checkbox
      $('#terms_and_conditions').prop('disabled', true);
    } else {
      // if term content has no scrollbar, enable term condition checkbox
      $('#terms_and_conditions').prop('disabled', false);
    }
  });  

  // when terms and conditions checkbox is checked, hide terms and conditions modal and show signature and payment modal
  $('#terms_and_conditions').on('change', function(){
    if($(this).prop('checked') === true) {
      ...
      setTimeout(() => reloadSignatureCanvas(), 50);
    }
  });

  // if payment mode "bank transfer" is selected, display bank details
  $('[id^="payment_mode_"]').on('click', function(){
    if($(this).attr('id') !== 'payment_mode_bt') {
      $('#bank_transfer_container').hide();  
    } else if($(this).attr('id') === 'payment_mode_bt') {
      if($(this).prop('checked') === true) {
  ...
  // when save button (not a submit button) is clicked, check validation of signature and payment.
  // if validations satisfy, click the hidden submit button in the signature and payment modal
  $('#signatureModal-click-submit').on('click', function(){
    var signatureValid  = false;
    var paymentValid    = false;
    
    if($('#signature_dataurl').val() === '') {
        // if signature dataurl is empty, display signature validation message
        $('#signature_validation').text($('#js_validation_signature_data_notEmpty').val());
    } else {
        signatureValid  = true;
        $('#signature_validation').html('');
    }

    $('input:radio[name="payment_mode"]').each(function(){
        if($(this).prop('checked') === true) {
            paymentValid  = true;
        }
    });
    ...
    if(paymentValid  === false) {
        // if payment mode is not checked, display payment mode validation message
      $('#payment_mode_validation').text($('#js_validation_payment_mode_notEmpty').val())
    } else {
      $('#payment_mode_validation').text('')
    }

    if(signatureValid === true && paymentValid === true) {
        $('#signatureModal-submit').click();
    }
  });

  $('input:radio[name="payment_mode"]').on('click', function(){
      var paymentModeSelected = false;
      if($(this).prop('checked') === true) {
          paymentModeSelected  = true;
      }
      if(paymentModeSelected === true) {
          $('#payment_mode_validation').text('')
      }
  });

  // when signature modal is displayed, set the signature pad
  function reloadSignatureCanvas() {
    $('#signature_pad').width('500');  // set the signature pad size
    var signature = $('#signature_pad').signature({ 
                                    change: function(event, ui) {
                                        $('#capture_signature').click(); // force buton to capture signature data url
                                    }
    ...
    $('#clear_signature').click(function() {
        ...
        $('#signature_dataurl').val('');
    });
    
    $('#capture_signature').click(function() {
        // convert the signature image to dataURL to save in DB
        if(signature.signature('isEmpty') === false) {
            var signatureDataUrl        = signature.signature('toDataURL'); // data:image/jpeg;base64,
            var signatureEncryptedData  = signatureDataUrl.split('base64,')[1];
            $('#signature_dataurl').val(signatureEncryptedData);
            $('#signature_validation').html('');
        }
    });    
  }
    ...
    const FormValidationGeneralInfoData = FormValidation.formValidation(stepGeneralInfoDataDataValidation, {
      ...
      if(validationStepper._currentIndex === 7) {
        ...
        if($('#terms_and_conditions').prop('checked') === true 
            && $('#signature_dataurl').val() !== ''
            && paymentValid === true) {
          // && $('input:radio[name="payment_mode"]').prop('checked') === true
        ...
        if(validateAllFields === false) {
            if($('#terms_and_conditions').prop('checked') === false) {
                ...
                if(paymentValid === false || $('#signature_dataurl').val() === '') {
                    $('#signatureModal').modal('show');
                    setTimeout(() => reloadSignatureCanvas(), 50);
                }
            }
        } else if(validateAllFields === true) {
          //$(document).on('submit', '#' + $dynamicFormId, function(event) {
            ...
            var retUrl   = $baseURL + 'registration';
            ...
            $.ajax({
                ...
                success     : function (data) {
                    ...
                    if(typeof responseData.id !== null && typeof responseData.id !== undefined
                        && responseData.id !== null && responseData.id !== undefined) {
                      window.location.href = retUrl + '/add-ajax-return?status=' + encodeURIComponent(status) + '&message=' + encodeURIComponent(message) + '&id=' + encodeURIComponent(responseData.id);
                    } else {
                      window.location.href = retUrl + '/add-ajax-return?status=' + encodeURIComponent(status) + '&message=' + encodeURIComponent(message);
                    }
                },
