      ...
      plugins: {
        ...
        if(validationStepper._currentIndex === 7) {
          //$(document).on('submit', '#' + $dynamicFormId, function(event) {
            ...
            var returnUrl   = $baseURL + 'pre-enrolment';
            ...
            $.ajax({
                ...
                success     : function (data) {
                    ...
                    window.location.href = returnUrl + '?status=' + encodeURIComponent(status) + '&message=' + encodeURIComponent(message);
                },
                error: function(jqXHR, textStatus, errorThrown) {
                  var data = jqXHR.responseJSON;console.log(data);
                  if($.isEmptyObject(data.errors) == false) {
                      $.each(data.errors, function (key, value) {
                          $('#' + key).closest('.col-sm-6').find('.invalid-feedback').html('<span class="help-block">' + value + '</span>');
                      });
                  }
