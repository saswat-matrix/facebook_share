$(document).ready(function(){
    ...
    $("[data-duplicate-popup=true]").click(function () {
        var titleTxt    = $(this).attr('data-duplicate-title');
        var messageTxt  = $(this).attr('data-duplicate-message');
        var link        = $(this).attr('data-duplicate-link');
        $('#duplicateModal-title').html(titleTxt);
        $('#duplicateModal-message').html(messageTxt);
        $("#duplicateModal").modal('show');
        $("#duplicateModal-ok").click(function () {
            window.location.href = link;
        })
        return false;
    });

    $("[data-approve-popup=true]").click(function () {
        var titleTxt    = $(this).attr('data-approve-title');
        var messageTxt  = $(this).attr('data-approve-message');
        var link        = $(this).attr('data-approve-link');
        $('#approveModal-title').html(titleTxt);
        $('#approveModal-message').html(messageTxt);
        $("#approveModal").modal('show');
        $("#approveModal-ok").click(function () {
            window.location.href = link;
        })
        return false;
    });
    ...
    $("[data-archive-popup=true]").click(function () {
        var titleTxt    = $(this).attr('data-archive-title');
        var messageTxt  = $(this).attr('data-archive-message');
        var link        = $(this).attr('data-archive-link');
        $('#archiveModal-title').html(titleTxt);
        $('#archiveModal-message').html(messageTxt);
    ...
    $("[data-set-organization-popup=true]").click(function () {
        var titleTxt    = $(this).attr('data-set-organization-title');
        var messageTxt  = $(this).attr('data-set-organization-message');
        var link        = $(this).attr('data-set-organization-link');
        $('#setOrganizationModal-title').html(titleTxt);
        $('#setOrganizationModal-message').html(messageTxt);
        $("#setOrganizationModal").modal('show');
        $("#setOrganizationModal-ok").click(function () {
            window.location.href = link;
        })
        return false;
    });
        ...
        if ($('#image').get(0).files.length  > 0) {
            ...
            if (mimeIs[0] === 'image') {
                ...
                if ($('#image').get(0).files[0].size > imageSize) {
                    ...
                    var htmlMSG = '<b><span class="msg-text-error">' + $('#js_validation_image_modal_error_message_size').val() + '</span></b>';
                    $('#imageModal-title').text($('#js_validation_image_modal_title').val());
                    ...
                    img.onload = function () {
                        ...
                        if (imageWidth <= parseInt(imgW) && imageHeight <= parseInt(imgH)) {
                            ...
                            // Displaying error message in modal //
                            var htmlMSG = '<b><span class="msg-text-error">' + $('#js_validation_image_modal_error_message_resolution').val() + '</span></b>';
                            $('#imageModal-title').text($('#js_validation_image_modal_title').val());
                            $('#imageModal-message').html('');
                            $('#imageModal-message').append(htmlMSG);
                            $('#imageModal').modal('show');
                ...
                // Displaying error message in modal //
                var htmlMSG = '<b><span class="msg-text-error">' + $('#js_validation_image_modal_error_message_type').val() + '</span></b>';
                $('#imageModal-title').text($('#js_validation_image_modal_title').val());
                $('#imageModal-message').html('');
                $('#imageModal-message').append(htmlMSG);
                $('#imageModal').modal('show');
...
if($('#reset_file_button').length > 0) {
    $('#reset_file_button').on('click', function(){
        if($('#file').length > 0) {
            reset($('#file'));
            if ($('#old_file').length > 0) {
                ...
                $('#preview_file').attr('src', $baseURL + 'images/general_images/pre_img2.png');
            }
        }
    });
}
...
/*-- sets the locale sent fromt front end via ajax --*/
...
/*-- checks if two arrays have same elements irrespectie of their order --*/
