$(document).ready(function(){
    $('#selectAll_add').on('click', function(){
        if($(this).prop('checked') === true) {
            $('[id^="addPermission__"]').prop('checked' , true);
        } else if($(this).prop('checked') === false) {
            $('[id^="addPermission__"]').prop('checked' , false);
        }
    })
});
                ...
                const fv = FormValidation.formValidation(formAuthentication, {
                    fields: {
                        add_title: {
                            validators: {
                                notEmpty: {
                                    message: $('#js_validation_add_title_notEmpty').val()
                                },
                                stringLength: {
                                    min: 2,
                                    max: 255,
                                    message: $('#js_validation_add_title_stringLength').val()
                                },
                            }
                        },
                        'add_permission[]': {
                            validators: {
                                notEmpty: {
                                    message: $('#js_validation_add_permission_notEmpty').val()
                                }
                            }
                        }
                    },
